#' Simple quantile autoregressive forecaster based on `grf`
#'
#' A simple quantile autoregressive forecaster based on `grf`, to be used
...
#' @export
grf_forecaster = function(df, forecast_date, signals, incidence_period,
                               ahead, geo_type,
  ...
  if (!is.null(transform)) {
    stop('Transformations not implemented.') # TODO?
  ...
  # Identify params for grf training and prediction functions
  params = list(...)
  params$quantiles = tau

  cv = is.null(params$lambda) || length(params$lambda) > 1
  cv = FALSE # TODO: implement cv?
  if (cv) {
    stop('cv not yet implemented.') # TODO
  }
  else {
    train_fun = grf::quantile_forest
    predict_fun = grf:::predict.quantile_forest
  }
  ...
  for (i in 1:length(ahead)) {
    ...
    if (cv && cv_type == "forward") {
      stop('cv not yet implemented.') # TODO: implement cv?
      ...
      train_params$train_test_inds = train_test_inds
		...
		# Remove NA values present in x or y (from quantgen::setup_xyd)
